...
struct DatumDeserializer<'s, 'r, R> {
    schema: &'s Schema,
    reader: &'r mut R,
...
impl<'de, R: Read<'de>> Deserializer<'de> for DatumDeserializer<'_, '_, R> {
        ...
        V: Visitor<'de>,
    {
        match self.schema {
            ...
            Schema::Float => {
                visitor.visit_f32(self.reader.read_const_size_buf(f32::from_le_bytes)?)
            }
            Schema::Double => {
                visitor.visit_f64(self.reader.read_const_size_buf(f64::from_le_bytes)?)
            }
